...
import ModalTrashTahunAjaran from '../../components/operator/ModalTrashTahunAjaran';
...
const TahunAjaranPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedTA) {
        await tahunAjaranAPI.update(selectedTA.id, form);
      } else {
        await tahunAjaranAPI.store(form);
      }
      setShowModal(false);
      fetchData();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      await tahunAjaranAPI.destroy(selectedTA.id);
      setShowDeleteModal(false);
      setSelectedTA(null);
      showToast('Tahun ajaran berhasil dipindahkan ke Recycle Bin');
      fetchData();
    } catch (err) {
      setError(err.response?.data?.message || err.message);
      setShowDeleteModal(false);
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading && dataTahunAjaran.length === 0 ? (
      ...
      {showDeleteModal && selectedTA && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm p-8 text-center">
            ...
            <p className="text-sm text-slate-500 mb-6">Tahun ajaran <strong>{selectedTA.tahun}</strong> akan dipindahkan ke Recycle Bin.</p>
