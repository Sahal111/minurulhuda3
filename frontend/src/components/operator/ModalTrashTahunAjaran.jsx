...
import { X, Trash2, Undo2, ChevronLeft, ChevronRight, Info, Calendar } from 'lucide-react';
import { tahunAjaranAPI } from '../../api/operator';

const ModalTrashTahunAjaran = ({ isOpen, onClose, onSuccess }) => {
    ...
    const fetchTrash = async (p = 1) => {
        setLoading(true);
        try {
            const res = await tahunAjaranAPI.trash({ page: p });
            const { tahun_ajarans } = res.data;
            setData(tahun_ajarans?.data || []);
            setPagination({
                current_page: tahun_ajarans?.current_page ?? 1,
                last_page: tahun_ajarans?.last_page ?? 1,
                total: tahun_ajarans?.total ?? 0,
                from: tahun_ajarans?.from ?? 0,
                to: tahun_ajarans?.to ?? 0,
            });
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const handleRestore = async (id, tahun) => {
        if (!window.confirm(`Pulihkan tahun ajaran "${tahun}"?`)) return;
        setProcessing(id);
        try {
            await tahunAjaranAPI.restore(id);
            onSuccess?.(`Tahun ajaran "${tahun}" berhasil dipulihkan`);
            fetchTrash(page);
        } catch (err) {
    ...
    const handleForceDelete = async (id, tahun) => {
        if (!window.confirm(`Hapus permanen "${tahun}"? Data TIDAK dapat dipulihkan!`)) return;
        setProcessing(id);
        try {
            await tahunAjaranAPI.forceDelete(id);
            onSuccess?.(`Tahun ajaran "${tahun}" dihapus permanen`);
            fetchTrash(page);
        } catch (err) {
                ...
                <div className="relative w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
                    <div className="px-8 pt-8 pb-6 flex items-center justify-between shrink-0 border-b border-slate-100">
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Tahun ajaran yang dihapus — dapat dipulihkan</p>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8">
                        {loading ? (
                            ...
                            <div className="text-center py-12 text-slate-400">
                                ...
                                <p className="text-xs mt-1">Tidak ada tahun ajaran yang dihapus</p>
                            </div>
                        ) : (
                                ...
                                {data.map(item => (
                                    <div key={item.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-bold text-slate-800 truncate">{item.tahun}</p>
                                            ...
                                            {item.semesters && item.semesters.length > 0 && (
                                                <div className="flex flex-wrap gap-1 mt-1">
                                                    {item.semesters.map(sem => (
                                                        <span key={sem.id} className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-slate-100 text-slate-500">{sem.nama}</span>
                                                    ))}
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex gap-2 shrink-0">
                                            <button onClick={() => handleRestore(item.id, item.tahun)} disabled={processing === item.id}
                                            ...
                                            <button onClick={() => handleForceDelete(item.id, item.tahun)} disabled={processing === item.id}
...
export default ModalTrashTahunAjaran;
