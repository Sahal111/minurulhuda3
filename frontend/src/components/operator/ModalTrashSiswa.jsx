...
import { X, Trash2, Undo2, ChevronLeft, ChevronRight, Info, AlertTriangle, User } from 'lucide-react';
import { siswaAPI } from '../../api/operator';

const ModalTrashSiswa = ({ isOpen, onClose, onSuccess, kelas = [] }) => {
    ...
    const fetchTrash = async (p = 1) => {
        setLoading(true);
        try {
            const res = await siswaAPI.trash({ page: p });
    ...
    const handleRestore = async (id, nama) => {
        if (!window.confirm(`Pulihkan data siswa "${nama}"?`)) return;
        setProcessing(id);
        try {
            await siswaAPI.restore(id);
            onSuccess?.(`Siswa "${nama}" berhasil dipulihkan`);
            fetchTrash(page);
        } catch (err) {
    ...
    const handleForceDelete = async (id, nama) => {
        ...
        setProcessing(id);
        try {
            await siswaAPI.forceDelete(id);
            onSuccess?.(`Siswa "${nama}" dihapus permanen`);
            fetchTrash(page);
        } catch (err) {
                ...
                <div className="relative w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
                    <div className="px-8 pt-8 pb-6 flex items-center justify-between shrink-0 border-b border-slate-100">
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Data siswa yang dihapus — dapat dipulihkan</p>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8">
                        {loading ? (
                            ...
                            <div className="text-center py-12 text-slate-400">
                                ...
                                <p className="text-xs mt-1">Tidak ada data siswa yang dihapus</p>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {data.map(s => (
                                    <div key={s.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                        {s.foto ? (
                                            <img src={`${apiBase}/storage/${s.foto}`} alt={s.nama} className="w-10 h-10 rounded-xl object-cover" />
                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[10px] text-slate-400">{s.nisn || '-'} · {s.nis || '-'}</p>
                                        </div>
                                        <div className="flex gap-2 shrink-0">
                                            <button onClick={() => handleRestore(s.id, s.nama)} disabled={processing === s.id}
                                            ...
                                            <button onClick={() => handleForceDelete(s.id, s.nama)} disabled={processing === s.id}
...
export default ModalTrashSiswa;
