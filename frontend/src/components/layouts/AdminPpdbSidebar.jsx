...
import {
    LayoutDashboard, Users, FileText, DollarSign, Settings, Shield
} from 'lucide-react';

const AdminPPDBSidebar = ({ sidebarOpen, setSidebarOpen, sidebarCollapsed }) => {
        ...
        <>
            ...
            <aside className={`fixed top-0 left-0 h-full bg-emerald-900 text-white z-50 transition-all duration-300 ease-in-out overflow-hidden flex flex-col shadow-2xl ${
                ...
                <div className="p-6 flex items-center gap-4 border-b border-emerald-800">
                    ...
                    {!sidebarCollapsed && (
                        <div className="truncate">
                            ...
                            <p className="text-[10px] text-emerald-300 uppercase tracking-widest">Admin PPDB</p>
                        </div>
                    )}
                </div>
                <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
                    ...
                    <NavLink to="/admin-ppdb/dashboard" className={linkClasses}>
                        <LayoutDashboard className="w-5 h-5 shrink-0" />
                        {!sidebarCollapsed && <span className="font-medium text-sm">Dashboard</span>}
                    </NavLink>
                    {!sidebarCollapsed && (
                        <p className="px-4 text-[10px] font-semibold text-emerald-400 uppercase tracking-widest mb-2 mt-6">PPDB</p>
                    )}
                    <NavLink to="/admin-ppdb/pendaftar" className={linkClasses}>
                        <Users className="w-5 h-5 shrink-0" />
                        {!sidebarCollapsed && <span className="font-medium text-sm">Pendaftar</span>}
                    </NavLink>
                    <NavLink to="/admin-ppdb/verifikasi" className={linkClasses}>
                        <Shield className="w-5 h-5 shrink-0" />
                        {!sidebarCollapsed && <span className="font-medium text-sm">Verifikasi Berkas</span>}
                    </NavLink>
                    <NavLink to="/admin-ppdb/pembayaran" className={linkClasses}>
                        <DollarSign className="w-5 h-5 shrink-0" />
                        {!sidebarCollapsed && <span className="font-medium text-sm">Pembayaran</span>}
                    </NavLink>
                    <NavLink to="/admin-ppdb/laporan" className={linkClasses}>
                    ...
                    <NavLink to="/admin-ppdb/setting" className={linkClasses}>
...
export default AdminPPDBSidebar;
